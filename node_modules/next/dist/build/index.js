...
async function build(dir, reactProductionProfiling = false, debugOutput = false, runLint = true, noMangling = false, appDirOnly = false) {
    try {
        ...
        const buildResult = await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            const pagesPaths = !appDirOnly && pagesDir ? await nextBuildSpan.traceChild("collect-pages").traceAsyncFn(()=>(0, _recursiveReaddir).recursiveReadDir(pagesDir, new RegExp(`\\.(?:${config.pageExtensions.join("|")})$`))) : [];
            let appPaths;
            if (appDir) {
                appPaths = await nextBuildSpan.traceChild("collect-app-paths").traceAsyncFn(()=>(0, _recursiveReaddir).recursiveReadDir(appDir, new RegExp(`^(page|route)\\.(?:${config.pageExtensions.join("|")})$`)));
            }
            ...
            const hasPages404 = (ref5 = mappedPages["/404"]) == null ? void 0 : ref5.startsWith(_constants.PAGES_DIR_ALIAS);
            ...
            if (config.experimental.clientRouterFilter) {
                const nonInternalRedirects = redirects.filter((redir)=>!redir.internal);
                const clientRouterFilters = (0, _createRouterClientFilter).createClientRouterFilter(appPageKeys, nonInternalRedirects);
            ...
            const requiredServerFiles = nextBuildSpan.traceChild("generate-required-server-files").traceFn(()=>({
                    ...
                    files: [
                        ...
                        _path.default.join(_constants1.SERVER_DIRECTORY, _constants1.FONT_LOADER_MANIFEST + ".js"),
                        _path.default.join(_constants1.SERVER_DIRECTORY, _constants1.FONT_LOADER_MANIFEST + ".json"),
            ...
            const { duration: webpackBuildDuration , turbotraceContext  } = await (0, _webpackBuild).webpackBuild();
            ...
            async function runTurbotrace(staticPages) {
                ...
                if (!(binding == null ? void 0 : binding.isWasm) && typeof binding.turbo.startTrace === "function") {
                    ...
                    let turbotraceFiles;
                    turboTasks = binding.turbo.createTurboTasks((((ref = config.experimental.turbotrace) == null ? void 0 : ref.memoryLimit) ?? _constants1.TURBO_TRACE_DEFAULT_MEMORY_LIMIT) * 1024 * 1024);
            ...
            if (config.experimental.preCompiledNextServer) {
                ...
                await _fs.promises.rename(nextServerPath, `${nextServerPath}.bak`);
                await _fs.promises.writeFile(nextServerPath, `module.exports = require('next/dist/compiled/next-server/next-server.js')`);
                const glob = require("next/dist/compiled/glob");
                const compiledFiles = [];
                const compiledNextServerFolder = _path.default.dirname(require.resolve("next/dist/compiled/next-server/next-server.js"));
                for (const compiledFolder of [
                    compiledNextServerFolder,
                    _path.default.join(compiledNextServerFolder, "../react"),
                    _path.default.join(compiledNextServerFolder, "../react-dom"),
                    _path.default.join(compiledNextServerFolder, "../scheduler"), 
                ]){
                    ...
                    await Promise.all(globResult.map(async (file)=>{
                        const absolutePath = _path.default.join(compiledFolder, file);
                        const statResult = await _fs.promises.stat(absolutePath);
                ...
                await _fs.promises.writeFile(nextServerTraceOutput, JSON.stringify({
                    ...
                    ].map((file)=>_path.default.relative(distDir, file))
                }));
                ...
                await nextBuildSpan.traceChild("trace-next-server").traceAsyncFn(async ()=>{
                    ...
                    const root = (((ref = config.experimental) == null ? void 0 : (ref14 = ref.turbotrace) == null ? void 0 : ref14.contextDirectory) ?? ((ref15 = config.experimental) == null ? void 0 : ref15.outputFileTracingRoot)) ?? dir;
            ...
            if (combinedPages.length > 0 || useStatic404 || useDefaultStatic500 || isAppDirEnabled) {
                ...
                await staticGenerationSpan.traceAsyncFn(async ()=>{
                    ...
                    for (const page2 of staticPages1){
                        const serverBundle = (0, _require).getPagePath(page2, distDir);
                    ...
                    for (const [originalAppPath1, routes1] of appStaticPaths){
                        ...
                        if (!hasDynamicData && (0, _utils).isDynamicRoute(originalAppPath1)) {
                            ...
                            finalDynamicRoutes[page] = {
                                ...
                                dataRouteRegex: isRouteHandler ? null : (0, _loadCustomRoutes).normalizeRouteRegex((0, _routeRegex).getNamedRouteRegex(dataRoute.replace(/\.rsc$/, "")).re.source.replace(/\(\?:\\\/\)\?\$$/, "\\.rsc$"))
                            };
                        }
                    }
                    const moveExportedPage = async (originPage, page, file, isSsg, ext, additionalSsgFile = false)=>{
                        return staticGenerationSpan.traceChild("move-exported-page").traceAsyncFn(async ()=>{
                            ...
                            const pagePath = (0, _require).getPagePath(originPage, distDir);
            ...
            await nextBuildSpan.traceChild("print-tree-view").traceAsyncFn(()=>(0, _utils1).printTreeView(pageKeys, allPageInfos, {
                    distPath: distDir,
                    buildId: buildId,
                    pagesDir,
                    useStatic404,
