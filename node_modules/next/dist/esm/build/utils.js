...
import { loadRequireHook, overrideBuiltInReactPackages } from "./webpack/require-hook";
...
export async function buildAppStaticPaths({ page , configFileName , generateParams  }) {
...
export async function isPageStatic({ page , distDir , configFileName , runtimeEnvConfig , httpAgentOptions , enableUndici , locales , defaultLocale , parentId , pageRuntime , edgeInfo , pageType , hasServerComponents , originalAppPath  }) {
...
export async function copyTracedFiles(dir, distDir, pageKeys, appPageKeys, tracingRoot, serverConfig, middlewareManifest) {
    ...
    for (const page of Object.values(middlewareManifest.functions)){
        ...
        for (const file1 of [
            ...
            const originalPath = path.join(distDir, file1.filePath);
            const fileOutputPath = path.join(outputPath, path.relative(tracingRoot, distDir), file1.filePath);
            await fs.mkdir(path.dirname(fileOutputPath), {
                recursive: true
            });
            await fs.copyFile(originalPath, fileOutputPath);
    ...
    for (const page1 of pageKeys){
        ...
        const pageFile = path.join(distDir, "server", "pages", `${normalizePagePath(page1)}.js`);
    ...
    await fs.writeFile(serverOutputPath, `${moduleType ? `import Server from 'next/dist/server/next-server.js'
...
export function getPossibleInstrumentationHookFilenames(folder, extensions) {
    return extensions.map((extension)=>path.join(folder, `${INSTRUMENTATION_HOOK_FILENAME}.${extension}`));
}
